...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const setHovered = (col: number, row: number) => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (isHex) {
        ...
        setHovered(col, Math.round((mouseY - offsetY - rowOffset) / hexVert));
      } else if (isTri) {
        const halfW = squareSize / 2;
        const offsetX = ((gridOffset.current.x % halfW) + halfW) % halfW;
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        setHovered(Math.round((mouseX - offsetX) / halfW), Math.floor((mouseY - offsetY) / squareSize));
      } else {
        const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        const round = shape === "circle" ? Math.round : Math.floor;
        setHovered(round((mouseX - offsetX) / squareSize), round((mouseY - offsetY) / squareSize));
      }
    };
